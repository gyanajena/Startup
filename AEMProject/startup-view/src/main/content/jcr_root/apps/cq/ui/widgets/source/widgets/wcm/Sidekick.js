...
//                    console.log(e);
    ...
    getPublishConfig: function(currentPage) {
        ...
        var allowed = CQ.User.getCurrentUser().hasPermissionOn("replicate", this.getPath() || this.path);
        return {
            "text": this.publishText,
            // Added permission check
            "hidden": currentPage.isLaunch() || !allowed,
            "handler": function() {
                var sidekick = CQ.wcm.Sidekick.findSidekick(this);
                var data = {
    ...
    loadWorkflowTab: function() {
        ...
        //to set the options later on 
        var _workflowConsolePanelSelection;
        var sendAJAX = true;
        ...
        if (!isInWorkflow) {
            ...
            workflowConsolePanel.items = [
                {
                    ...
                    "listeners": {
                        ...
                        "afterrender": function() {
                            if (typeof pageInfo.workflows === "undefined") {
                                if (sendAJAX === true) {
                                    sendAJAX = false;
                                    var selection = this;
                                    var url = CQ.HTTP.externalize("/libs/cq/workflow/content/wfmodels.wcmtranslation.json");
                                    $.ajax({
                                        url: url,
                                        type: 'GET'
                                        ...
                                        //render the WCM Workflow Options
                                        var wcmOptions = CQ.WCM.getSidekick().getWorkflowModels(response,"wcm");
                                        pageInfo.workflows.wcm = wcmOptions;
                                        selection.setOptions(wcmOptions);

                                        //render the Transtaltion Workflow Options
                                        if(_workflowConsolePanelSelection && _workflowConsolePanelSelection.setOptions) {
                                            var translationOptions = CQ.WCM.getSidekick().getWorkflowModels(response,"translation");
                                            pageInfo.workflows.translation = translationOptions;
                                            _workflowConsolePanelSelection.setOptions(translationOptions);
                                        }
                                    });
                                } else {
                                    _workflowConsolePanelSelection = this;
                                }
                            } else {
                                var translationOptions = CQ.WCM.getSidekick().getWorkflowModels(pageInfo.workflows,"translation");
            ...
            if (!showWorkflowActions) {
                ...
                var fnGetWorkItemDialog = function(wiPath) {
                    var pi = CQ.WCM.getPageInfo(CQ.WCM.getPagePath());
                    var dlgPath = "";
...
//            console.log("Sidekick::loadTranslation: "+e);
                                    ...
                                    if (sendAJAX === true) {
                                        ...
                                        $.ajax({
                                            ...
                                            if(_workflowConsolePanelSelection && _workflowConsolePanelSelection.setOptions) {
                                                var wcmOptions = CQ.WCM.getSidekick().getWorkflowModels(response,"wcm");
                                                pageInfo.workflows.wcm = wcmOptions;
                                                _workflowConsolePanelSelection.setOptions(wcmOptions);
                                            }

                                            //render the Transtaltion Workflow Options
                                            var translationOptions = CQ.WCM.getSidekick().getWorkflowModels(response,"translation");
                                            pageInfo.workflows.translation = translationOptions;
                                            selection.setOptions(translationOptions);
                                        });
                                    } else {
                                        _workflowConsolePanelSelection = this;
                                    }
    ...
    /**
     * Returns list of workflow models to be displayed in the selection component
     * @private
     * @return {Object} List of workflow models
     */
    getWorkflowModels: function(workflowModels, modelName){
        var workflowOptions = [];
        var wData = workflowModels && workflowModels[modelName] ? workflowModels[modelName].models : [];
